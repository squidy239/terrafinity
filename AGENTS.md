...
- **Units:** the filter runs entirely in normalized space. `p` fed to `erosionFilter` is the noise-grid base coordinate _divided_ by `erosion_scale` (the reference's detail scale: `EROSION_SCALE = 0.15` puts gullies at ~0.15x the terrain feature size; multiplying instead makes gullies _coarser_ than the relief and looks wrong). The height input is the raw `shaped` field (≈[-1,1]) and the envelope `|bounds| * scale` is applied once to the output delta — never before the filter. The downhill slope is the negated central-difference gradient of `shaped` at +/- one sample spacing (`grad / erosion_scale` is a no-op under the full assumed-slope override since only the direction survives, but keep it for dimensional correctness).
...
- **Peak/valley preservation (steepness modulation):** the stripe frequency is scaled by the terrain steepness (the octave sampling becomes `p * freq * steepness`, and the `side` chain rule carries the same factor), so steep slopes keep fine, detailed gullies while flat terrain's stripes grow infinitely thick and the wave collapses to a constant — a summit or stream bed is never carved by a gully crossing it. `fade_slope` is the slope magnitude at which the stripes run at full frequency. The collapse lands on the wave's null (`cos(offset*tau)`; the port's offset 0.25 gives ~0), not the reference's white cell stripe, so flat ground is left _undisturbed_ instead of raised — no valley bulges. The first octave's `combi_mask` is seeded with the inverted-quadratic mask `1 - (1 - steepness)^2` (fully closed on flats, 3/4 open at half steepness), which scales the collapsed wave to exactly zero and keeps the flat-to-steep transition free of the folds a sqrt-style fade would leave near peaks. `fade_slope <= 0` disables the modulation. A pure coordinate-frequency shrink alone collapses to the pattern's arbitrary origin phase (up to half the ridge amplitude), so the mask must stay: it pins flat terrain to the null.
- **LOD consistency:** effective octaves = `clamp(erosion_octaves - level, 1, erosion_octaves)`; octaves stay absolute in world space. The test compares a level-1 chunk against box-filtered level-0 chunks (their sample coordinates are bit-exact after the `(1 + x/32) * 2` vs `2 + 2x/32` identity); tolerance must cover the gradient-resampling shift (e doubles per level), which costs ~5x the dropped-octave amplitude. The steepness modulation adds more: the coarser stencil also moves the per-sample frequency and mask (measured ~0.52 of the filter magnitude at the worst point), so the LOD bound is 55% of `filter_strength / (1 - gain) * gully_weight` plus fixed slack.
