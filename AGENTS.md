...
- **Units:** the filter runs entirely in normalized space. `p` fed to `erosionFilter` is the noise-grid base coordinate _divided_ by `erosion_scale` (the reference used a detail scale of 0.15, putting gullies at ~0.15x the terrain feature size; the default is ~3.29 for broad gullies — multiplying instead of dividing makes gullies _coarser_ than the relief and looks wrong). The height input is the raw `shaped` field (≈[-1,1]). On the way out the base field takes the asymmetric envelope `|bounds| * scale` (continuous through zero by construction), but the deltas must not: converting the erosion delta or the surface noise with the local `|bounds|` doubles them at the shoreline (`terrain_max` above the sea vs `|terrain_min|` below) and ends every crossing gully in a cliff. Both convert with the symmetric `max(|min|, |max|) * scale` instead (both flow through `composeErodedHeight`), so relief is identical on land and under water; both amplitudes stay in normalized units so saved configs keep their magnitude. The downhill slope is the negated central-difference gradient of `shaped` at +/- one sample spacing (`grad / erosion_scale` is a no-op under the full assumed-slope override since only the direction survives, but keep it for dimensional correctness).
- **Cell size is a locked ratio, never an absolute size:** inside `phacelleNoise` the phase ramps at `cell_scale * TAU` per unit cell (one cell holds exactly `cell_scale` stripes), and the octave `freq` scales the stripe frequency and the cell grid simultaneously (cells are unit-sized in `p * freq` space). Doubling the stripe frequency therefore halves the cells automatically, so the video's "fixed ratio across octaves" rule needs no per-octave cell tuning — `cell_scale` is a unitless ratio (0.7 = 0.7 stripes per cell) layered on the `erosion_scale` master. Ratios well below 1 leave under a full wave per cell and drift toward the grainy "cell too small" regime; the 4x4 cell blending (`bellWeight`) absorbs the slack, so the exact value is a tuning choice.
- **Load-bearing order:** each octave steers off `gully_slope` accumulated by the previous octave; `phacelle.side *= -freq` is the chain rule for the caller's `p * freq` scaling plus the downhill sign flip. `side` returned by `phacelleNoise` carries `norm_dir_perp * cell_scale * TAU`; the caller scales it by the octave `freq` exactly once.
- **Mask chain:** `combi_mask = powInv(combi_mask, detail) * new_mask` with `powInv(x, p) = x^(1/p)` (detail < 1 crushes older masks, restricting fine octaves to steep slopes). `smoothStart(t, smoothing)` divides by `smoothing` (smoothstep-shaped) — `crease_rounding = 0` makes that a division by zero, so it must guard `smoothing <= 0` and return a step. `easeOut` is a plain smoothstep ramp; the per-octave `new_mask` gates on the _wave's_ slope (`|sin|`), while the first-octave seed is the terrain steepness mask (see the next bullet).
- **Peak/valley preservation (steepness fade):** the octave sampling stays absolute in world space; flats are protected by the mask alone. The first octave's `combi_mask` is seeded with the inverted-quadratic mask `1 - (1 - steepness)^2` (fully closed on flats, 3/4 open at half steepness), and `powInv(0)` latches it closed, so on flat ground every octave lerps to the previous target (zero on the first) and the height delta is exactly 0 — a summit or stream bed is never carved. `fade_slope` is the steepness at which the mask runs fully open. The steepness itself comes from a wide-stencil average, not the per-sample gradient, or fine-noise flicker would strobe the mask between neighbors. `fade_slope <= 0` disables the fade.
- **LOD consistency:** effective octaves = `clamp(erosion_octaves - level, 1, erosion_octaves)`; octaves stay absolute in world space. The test compares a level-1 chunk against box-filtered level-0 chunks (their sample coordinates are bit-exact after the `(1 + x/32) * 2` vs `2 + 2x/32` identity); tolerance must cover the gradient-resampling shift (e doubles per level) plus the coarser fade stencil moving the per-sample mask, so the LOD bound is 40% of `filter_strength / (1 - gain) * gully_weight` plus 128 blocks fixed slack.
- **Hash deviation:** the reference's `fract`-based cell jitter loses precision far from the origin; `phacelle.zig` uses `fastnoise.hash2D`/`hash2DVec` (file-scope pub helpers primed internally with `prime_x`/`prime_y`) with the low/high 16 bits mapped to [-0.5, 0.5]. Keeps the pattern exact at ±100,000-block coordinates.
- **Benchmark (Debug, Xeon):** base `genTerrainHeight` ≈ 540 µs/chunk; with the filter ≈ 6.4 ms/chunk when `erosion_enabled` — the filter is ~12x the whole base pipeline. The gradient stage is ~4 extra batched warp+noise fills (the noise part dominates); Phacelle's 16 cells x 4 octaves of transcendentals dominate the rest. Reduction levers in order: polynomial `exp(-2d^2)` weight (**done**: `bellWeight` in `phacelle.zig`, Chebyshev fit to ~3e-7), fused sincos (not exposed by Zig), LOD octave drop (**done**), `@Vector(16, f32)` cell loop (**done**: the 4x4 neighbourhood runs as one vector op per wave term; Debug `genTerrainHeight` with erosion ≈ 5.2 ms/chunk vs 8.3 ms before).
- **Erosion direction must be build-stable:** the `grad_x`/`grad_z` accumulators were `undefined`, and `addGradientSamples`' first pass reads them (`grad_row + shaped*k`). Uninitialized stack content is build- and layout-dependent, so ReleaseFast vs Debug got different per-chunk gradient offsets; on low-slope terrain the direction is cancellation-dominated anyway, and the whole gully pattern rotated between builds (the "square pattern" in ReleaseFast). Fixes: zero-init the accumulators (`@splat(0)`), use `@mulAdd` for the accumulation so it cannot FMA-contract differently per build, and blend the seeded direction toward the assumed-slope direction below the 1e-4 gradient noise floor (`gradient_floor` in `erosionFilter`), because the fastnoise fills run `@setFloatMode(.optimized)` and are never bit-stable across build modes.
- **Config compat:** `erosion_strength` is kept in `Params` with `.skip = true` in `field_specs`; the superseded `erosion_fade_altitude` (the altitude fade target, rejected in favor of the steepness modulation) follows the same pattern — kept with a declaration default and `.skip = true` so saved configs that set it still parse. `std.zon.parse` only tolerates missing fields that carry a **declaration default** (`= value` in the struct field). The old `Params` kept every default in the `.default` const, so any config missing a field — including the game's own emitted file, which omits skipped fields — failed to parse; `loadConfig` silently fell back to the default preset, and `Game.init` re-saved that preset over the user's file on the next boot. Fixes: every `Params` field now carries its default in the declaration (`.default` is just `Params{}`), so old configs (missing the new `erosion_*` fields) and emitted configs (missing `erosion_strength`) parse cleanly; and `loadConfig` returns `error.InvalidConfig` instead of falling back, so a genuinely broken config fails loudly without rewriting the file. Any field added to `Params` must keep a declaration default or old saved configs break the same way.
