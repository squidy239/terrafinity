...
//! basis, scene-AABB depth range, texel snapping, camera-relative matrix emission,
//! origin compensation, and the refresh scheduler — pure math, fully unit-tested.

const std = @import("std");
const testing = std.testing;

/// Maximum cascade count supported by the CPU-side math.
...
/// PSSM log term degenerates below 1.0, so splits are clamped to at least this value.
...
/// Shadow quality and cost knobs. Live-editable: resource-affecting changes need
/// `recreate()`, the rest apply on the next frame.
/// Defaults mirror the shipped Config.zon.
pub const ShadowConfig = struct {
    enabled: bool = true,
    cascade_count: u32 = 12,
    shadow_map_size: u32 = 4096,
    depth_format: enum { d16, d32 } = .d32,
    ...
    max_shadow_distance: f32 = 128000,
    pssm_lambda: f32 = 1.0,
    ...
    /// farther ones rotate across the budget, never exceeding it (see `nextRefreshSet`).
    cascades_per_frame: u32 = 1,
...
/// dawn/dusk. Callers gate all shadow work on it being positive.
...
fn dot3d(a: Vec3d, b: Vec3d) f64 {
    return @reduce(.Add, a * b);
}

fn cross3f(a: Vec3f, b: Vec3f) Vec3f {
    return .{
        a[1] * b[2] - a[2] * b[1],
        a[2] * b[0] - a[0] * b[2],
        a[0] * b[1] - a[1] * b[0],
...
/// latched `current` basis. Holding the basis frozen below the step keeps every cascade
/// re-snapping onto an identical texel grid as the sun drifts (no shadow swimming);
/// stepping only on discrete changes re-orients the whole set in one clean increment.
...
/// Practical split radii from the plan's PSSM formula: the first split equals `near`
/// and the last equals `far`; entries past `count` are undefined and must not be read.
/// Live-edited configs are sanitized rather than asserted, so an absurdly small distance
/// or zero count degrades to a minimal usable set instead of panicking.
fn pssmSplits(near: f32, far: f32, count: u32, lambda: f32) [max_cascades]f32 {
...
/// Effective per-cascade outer split radii from PSSM: the nearest sits exactly on
/// `min_split_radius`, the outer on `max_shadow_distance`. Degenerate live-edited
/// configs are sanitized, not asserted.
...
fn snapCenter(center: Vec3d, radius: f32, shadow_map_size: u32, basis: LightBasis) Vec3d {
...
/// the compute cull share it, so no depth-clamped false occluders paint the map's edges;
/// the pipeline's depthClamp only affects geometry straddling the range boundary.
fn depthRange(scene_min: Vec3d, scene_max: Vec3d, center: Vec3d, light_dir: Vec3f, radius: f32, max_depth_range: f32) struct { near: f32, far: f32 } {
    ...
    // receivers themselves out of range.
...
/// most-overdue first with ties toward the near cascades, and at most `cascades_per_frame`
/// are picked — when nothing is due the set is empty and the shadow pass is skipped.
...
fn stalenessPadding(staleness_frames: u32, per_frame_dist: f32) f32 {
    return @as(f32, @floatFromInt(staleness_frames)) * @max(per_frame_dist, 0.0);
}

/// Emits the camera-relative light view-projection for a committed cascade directly in
/// GLSL column-major layout (translation in the 4th column, bottom row (0,0,0,1)) so w
/// stays 1 — zm's row-major lookAtRH/ortho would read the off-origin translation as a
/// projective w term. The eye sits at `center - view_pos` (f64 subtract) so the result
/// matches `MeshData.relative_position` exactly; Vulkan NDC depth [0, 1] maps the near
/// plane to 0 and the far to 1, matching the shadow pass's LESS_OR_EQUAL compare.
fn buildViewProj(committed: CommittedCascade, view_pos: Vec3d) [16]f32 {
    const basis = buildLightBasis(committed.light_dir, world_up);
    const s = basis.right;
    const u = basis.up;
    const f = basis.forward;
    const fnf = committed.far_plane - committed.near_plane;
    const t: Vec3f = .{ @floatCast(view_pos[0] - committed.center_abs[0]), @floatCast(view_pos[1] - committed.center_abs[1]), @floatCast(view_pos[2] - committed.center_abs[2]) };
    const s_t = dot3f(s, t);
    const u_t = dot3f(u, t);
    const f_t = dot3f(f, t);
    var m: [16]f32 = @splat(0.0);
    // Vector element access needs a comptime index, so the loop stays inline.
    inline for (0..3) |c| {
        m[c * 4] = s[c] / committed.radius;
        m[c * 4 + 1] = u[c] / committed.radius;
        m[c * 4 + 2] = f[c] / fnf;
    }
    m[12] = s_t / committed.radius;
    m[13] = u_t / committed.radius;
    m[14] = (f_t - committed.near_plane) / fnf;
    m[15] = 1.0;
    return m;
}
...
pub fn computeCascade(ctx: CascadeContext, cascade_index: u32) Cascade {
    ...
    const committed = CommittedCascade{ .center_abs = center_snapped, .radius = radius, .near_plane = range.near, .far_plane = range.far, .light_dir = ctx.light_dir };
    ...
    return .{
        ...
        .viewproj = buildViewProj(committed, ctx.view_pos),
    };
}
...
/// correctly on frame N+1; the eye is placed at `center - view_pos` (f64 subtract),
/// matching `MeshData.relative_position` exactly, rather than compensating by matrix
/// composition.
pub fn viewProjAtOrigin(committed: CommittedCascade, view_pos: Vec3d) [16]f32 {
    return buildViewProj(committed, view_pos);
}

/// Trims a computed cascade to the fields needed to rebuild its matrix at any origin.
...
test "rebuilt matrix at a new origin reproduces the committed one" {
    ...
    for (clip_committed, clip_rebuilt) |a, b| {
        try testing.expectApproxEqAbs(a, b, 1e-3);
    }
}
...
test "refreshIntervals pinned endpoints, monotonic, lambda extremes" {
    ...
    for (uniform[0..3], uniform[1..4]) |prev, next| try testing.expect(next >= prev);
    ...
    for (log[0..3], log[1..4]) |prev, next| try testing.expect(next >= prev);
...
test "nextRefreshSet respects budget, ramps never-refreshed cascades, no starvation" {
    ...
    for (second[0..4], last_refresh[0..4]) |picked, *lr| {
        if (picked) {
            lr.* = frame;
            second_count += 1;
        }
    ...
    for (0..64) |i| {
        ...
        for (set[0..4], last_refresh[0..4], refreshed[0..4], 0..) |picked, *lr, *r, c| {
            if (picked) {
                r.* = true;
                lr.* = frame;
