...
pub fn ChunkLoaderUnloaderThread(renderer: *Renderer, conn: ?zudp, intervel_ns: u64, running: *std.atomic.Value(bool)) void {
        ...
        while (y < distance[1]) {
            while (z < distance[2]) {
                ...
                if (!loading and !loaded) {
                    ...
                    renderer.pool.spawn(Renderer.AddChunkToRenderNoError, .{ renderer, ChunkPos }) catch |err| std.debug.panic("pool spawn failed: {any}\n", .{err});
                }
                z += 1;
            }
...
//fn UnloadChunks(world: *World, loadDistance: [3]u32) !void {}
