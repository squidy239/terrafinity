...
const RayIntersection = @import("./RayIntersection.zig");
const Textures = @import("./Blocks.zig").Textures;
const Blocks = @import("./Blocks.zig").Blocks;
const Chunk = @import("./Chunk.zig").Chunk;
const ChunkStates = @import("./Chunk.zig").ChunkState;
...
const pw = @import("./World.zig").pw;
const ChunkMesh = @import("./World.zig").ChunkMesh;
const DistanceOrder = @import("./World.zig").DistanceOrder;
...
pub fn main() !void {
    ...
    _ = glfw.setWindowSizeCallback(window, glfwSizeCallback);
    ...
    while (!window.shouldClose()) {
        ...
        const projview = @as(@Vector(16, f32), @floatCast(zm.Mat4.perspective(std.math.degreesToRadians(90.0), @as(f32, @floatFromInt(width)) / @as(f32, @floatFromInt(height)), 0.1, @floatFromInt(player.MeshDistance[0] * 32)).multiply(zm.Mat4.lookAt(@Vector(3, f32){ 0, 0, 0 }, @Vector(3, f32){ 0, 0, 0 } + player.cameraFront, player.cameraUp)).data));
...
export fn MouseCallback(window: *glfw.Window, xpos: f64, ypos: f64) void {
    ...
    if (cursor_disabled) {
        ...
        if (player.pitch < -89.9)
            player.pitch = -89.9;
        player.cameraFront[0] = @floatCast(@sin(std.math.degreesToRadians(player.yaw)) * @cos(std.math.degreesToRadians(player.pitch)));
        player.cameraFront[1] = @floatCast(@sin(std.math.degreesToRadians(player.pitch)));
        player.cameraFront[2] = @floatCast(@cos(std.math.degreesToRadians(player.yaw)) * @cos(std.math.degreesToRadians(player.pitch)));
    }
}
...
fn prossesInput(window: *glfw.Window, dt: f64) !void {
    ...
    if (player.gameMode != Entitys.GameMode.Spectator) {
        cf[1] = 0;
        cf = zm.vec.normalize(cf);
    }
    const cameraSpeed = @Vector(3, f32){ deltaTime, deltaTime, deltaTime } * player.speed;
