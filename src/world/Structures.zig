...
fn generateTrunk(editor: *WorldEditor, base: @Vector(3, i64), params: GiantTreeGenParams) !void {
    ...
    while (y < params.height) : (y += 1) {
        const r = getTaperedRadius(params.base_radius, y, params.height, params.top_radius_factor);
...
fn generateRoots(editor: *WorldEditor, base: @Vector(3, i64), params: GiantTreeGenParams) !void {
    ...
    while (root < params.num_roots) : (root += 1) {
        ...
        var step: u32 = 0;
        while (step < params.root_length) : (step += 1) {
            const t = @as(f32, @floatFromInt(step)) / @as(f32, @floatFromInt(params.root_length));
            const dist = @as(f32, @floatFromInt(params.base_radius)) + t * @as(f32, @floatFromInt(params.root_length));
            const h = @max(0, @as(i32, @intFromFloat(@as(f32, @floatFromInt(params.root_length)) * 0.5 * (1.0 - t * t))));
            const x = base[0] + @as(i64, @intFromFloat(@cos(angle) * dist));
            const z = base[2] + @as(i64, @intFromFloat(@sin(angle) * dist));
            try editor.PlaceBlock(.{ .block = .Wood, .pos = .{ x, base[1] + h, z } });
        }
    }
}
...
fn generateBranches(editor: *WorldEditor, base: @Vector(3, i64), params: GiantTreeGenParams) !void {
    ...
    while (branch < params.main_branches) : (branch += 1) {
        ...
        while (step < params.branch_length) : (step += 1) {
            ...
            const x = base[0] + @as(i64, @intFromFloat(@cos(angle) * dist));
            const z = base[2] + @as(i64, @intFromFloat(@sin(angle) * dist));
            const y = base[1] + @as(i64, @intFromFloat(start_y + t * @as(f32, @floatFromInt(params.branch_length)) * 0.4));
...
fn generateCanopy(editor: *WorldEditor, base: @Vector(3, i64), params: GiantTreeGenParams, rng: std.Random) !void {
    ...
    const center_y = base[1] + params.height + r / 3;
...
pub fn PlaceGiantTree(editor: *WorldEditor, base: @Vector(3, i64), rng: std.Random, params: GiantTreeGenParams) !void {
    ...
    trunk.End();
    const roots = ztracy.ZoneNC(@src(), "genroots", 6436);
    try generateRoots(editor, base, params);
    roots.End();
