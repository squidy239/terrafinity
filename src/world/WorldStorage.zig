...
pub fn getBlocks(source: World.ChunkSource, world: *World, blocks: *Chunk.BlockEncoding, Pos: World.ChunkPos) error{ Unrecoverable, OutOfMemory }!bool {
    ...
    switch (encoding) {
        .blocks => {
            blocks.toBlocks(&world.block_grid_pool, &world.block_grid_count, &world.block_grid_pool_mutex);
            buf_reader.readSliceEndian(Block, @as([]Block, @ptrCast(blocks.blocks)), .little) catch unreachable;
        },
        .oneBlock => blocks.merge(.{ .oneBlock = @enumFromInt(buf_reader.takeInt(BlockTagType, .little) catch unreachable) }, &world.block_grid_pool, &world.block_grid_count, &world.block_grid_pool_mutex),
    }
    rocksdb.free(value.?);
    return true;
}
